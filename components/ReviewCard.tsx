...
export default function ReviewCard({ userId }: ReviewCardProps) {
  ...
  const submit = async () => {
    if (rating === 0) { setError(t.selectStar); return }
    ...
    try {
      ...
      if (!res.ok) { setError(t.error); setSending(false); return }
      setSubmitted(true)
    } catch {
      setError(t.error)
      setSending(false)
  ...
  if (submitted) {
    return (
      <div className="bg-gradient-to-r from-violet-600/20 to-fuchsia-600/20 border border-violet-500/30 rounded-2xl p-6 mb-8 text-center">
        ...
        <p className="text-white font-semibold">{t.thanks}</p>
        <p className="text-white/50 text-sm mt-1">{t.thanksDesc}</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-[#1e293b] border border-white/10 rounded-2xl p-6 mb-8">
      <h3 className="text-white font-bold text-lg">{t.question}</h3>
      <p className="text-white/50 text-sm mb-4">{t.private}</p>
      ...
      <label className="flex items-start gap-3 cursor-pointer mt-4" onClick={() => setAllowPublic(!allowPublic)}>
        ...
        <span className="text-white/70 text-sm">{t.consent}</span>
      </label>
