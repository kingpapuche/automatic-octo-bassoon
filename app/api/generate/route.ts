...
const STYLE_PROMPTS: Record<string, string> = {
  ...
  'leaning-elegant':     '[TRIGGER], leaning sideways against a sleek modern architectural wall of large grey stone panels, one shoulder and upper arm pressed firmly against the wall with the body relaxed and tilted at an angle, clearly resting his weight on the wall, one hand in pocket, smart casual blazer over a shirt, soft natural light, approachable elegant confidence',
  ...
  'w-leaning-elegant':    '[TRIGGER], portrait of woman leaning sideways against a sleek modern architectural wall of large grey stone panels, one shoulder and upper arm pressed firmly against the wall with the body relaxed and tilted at an angle, clearly resting her weight on the wall, one hand in pocket, smart casual blazer, soft natural light, approachable elegant confidence',
  ...
  try {
    ...
    for (const styleId of styleIds) {
      ...
      const backgroundClause = CLEAN_BG_STYLES.has(styleId)
        ? ', a sleek premium architectural wall of large grey stone or polished concrete panels directly behind the subject, clean and uncluttered with subtle natural stone texture visible, upscale editorial backdrop, subject prominent and sharp, softly blurred depth of field'
      ...
      if (CLEAN_BG_STYLES.has(styleId)) {
        // Voor rustige stijlen: sta een clean stenen wand toe (verwijder 'plain empty/bare wall'),
        // maar blijf een goedkope vlakke muur en clutter weren.
        effectiveNegative = effectiveNegative.replace(', plain empty wall, boring flat background, bare wall background, dull background', ', boring flat background, dull background')
        styleNegative += ', cheap flat drywall, weak flat painted wall, furniture, plants, curtains, doorway, hallway, room clutter, busy interior, visible room details, framed pictures on the wall'
      }
